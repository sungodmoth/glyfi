...
use crate::server_data::{
    AMBIGRAM_SUBMISSION_CHANNEL_ID, GLYPH_SUBMISSION_CHANNEL_ID, SUBMIT_EMOJI_ID,
};
use crate::sql::{check_submission, check_user, get_current_week, register_user, Challenge};
use crate::{err, file, info, info_sync, sql};
...
async fn match_relevant_reaction_event(
    ctx: &Context,
    r: &Reaction,
) -> Option<(User, Message, Challenge, i64)> {
    ...
    let Ok(user) = r.user(&ctx).await else {
        return None;
    };
    let Ok(message) = r.message(&ctx).await else {
        return None;
    };
    ...
    let Ok(current_week) = get_current_week(challenge).await else {
        return None;
    };

    return Some((user, message, challenge, current_week));
}
...
impl EventHandler for GlyfiEvents {
    ...
    async fn reaction_add(&self, ctx: Context, r: Reaction) {
        let Some((user, message, challenge, current_week)) =
            match_relevant_reaction_event(&ctx, &r).await
        else {
            return;
        };
